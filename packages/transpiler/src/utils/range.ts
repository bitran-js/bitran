...
export function range(start: number, end: number): Range {
    if (start < 0 || end < 0)
        throw new Error(
            `Error creating range: provided start '${start}' or end '${end}' can't be less than 0!`,
        );

    if (end <= start)
        throw new Error(
            `Error creating range: provided end '${end}' can't be less or equal start '${start}'!`,
        );
...
export function tryRange(start: number, end: number): Range | undefined {
    try {
...
export function getIntersection(
    ...
    if (rangeA.end < rangeB.start) return RangeIntersection.None;
